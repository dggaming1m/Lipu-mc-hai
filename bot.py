...
async def like_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        print(f"[DEBUG] Fetching player info from: {url}")
        ...
        if resp.status_code == 200:
            info = resp.json()
            player_name = info.get("name") or f"Player-{uid[-4:]}"
            region = info.get("region") or "?"
        else:
            player_name = f"Player-{uid[-4:]}"
            region = "?"
    except Exception as e:
        print(f"[ERROR] UID API failed: {e}")
        player_name = f"Player-{uid[-4:]}"
        region = "?"
...
Try again after: {hours}h {minutes}m"""
            ...
            try:
                ...
                if added == 0:
                    result = f"💔 UID {uid} ({player}) has already received max likes for today 😢. Try again tomorrow!"
                else:
